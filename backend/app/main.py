...
async def auth_epic_start(format: str = Query(default="redirect")) -> Any:
    ...
    if format == "json":
        return {"authorize_url": authorize_url, "state": signed_state}
...
async def auth_epic_callback(
    ...
    try:
        ...
        patient_id = str(session_payload["patientId"])
        ...
        auth_token = AuthToken(
            ...
            refresh_token=str(token_response.get("refresh_token") or "") or None,
            patient_id=patient_id,
            clinic_id=state_entry.clinic_id or "demo-clinic",
            expiry=expires_at.isoformat(),
            scope_list=str(token_response.get("scope") or "").split(),
...
async def _ingest_request(request: WorkflowIngestRequest) -> dict[str, Any]:
    try:
        session_id = (request.sessionId or "").strip() or None
        return await run_workflow_pipeline(
            ...
            session_id=session_id,
            session_cache=session_cache if session_id else None,
        )
    except Exception as exc:  # noqa: BLE001
